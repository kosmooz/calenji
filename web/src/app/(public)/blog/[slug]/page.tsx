...
function renderMarkdown(content: string): string {
  ...
  html = html.replace(/^### (.+)$/gm, (_, t) => {
    const id = t.replace(/\*\*/g, "").trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
    return `<h3 id="${id}">${t.replace(/\*\*/g, "")}</h3>`;
  });
  html = html.replace(/^## (.+)$/gm, (_, t) => {
    const id = t.replace(/\*\*/g, "").trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
    return `<h2 id="${id}">${t.replace(/\*\*/g, "")}</h2>`;
  });
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("<h") || line.startsWith("<hr")) { result.push(line); continue; }
    if (line.startsWith("> ")) { result.push(`<blockquote><p>${inlineFormat(line.slice(2))}</p></blockquote>`); continue; }
        ...
        <div className="container">
          <motion.div className="max-w-3xl mx-auto" initial="hidden" animate="visible" variants={fadeUp}>
            <motion.div variants={fadeUp} custom={0} className="flex items-center gap-3 mb-5">
              ...
              <span className="flex items-center gap-1 text-[13px] text-gray-400"><Clock size={13} /> {article.readTime}</span>
            </motion.div>
            <motion.h1 variants={fadeUp} custom={1} className="text-3xl md:text-[2.75rem] font-extrabold text-gray-900 tracking-tight mb-6 leading-[1.15]">{article.title}</motion.h1>
            <motion.p variants={fadeUp} custom={2} className="text-lg text-gray-500 mb-8 leading-relaxed">{article.excerpt}</motion.p>
            <motion.div variants={fadeUp} custom={3} className="flex items-center gap-6 pb-8 border-b border-gray-100">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center"><User size={16} className="text-gray-500" /></div>
                <div>
          ...
          <div className="max-w-3xl mx-auto flex gap-12">
            {headings.length > 3 && (
              <aside className="hidden xl:block w-56 shrink-0 -ml-64 sticky top-24 self-start max-h-[calc(100vh-8rem)] overflow-y-auto">
                <p className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-1.5"><BookOpen size={12} /> Sommaire</p>
                <nav className="space-y-1">
                  {headings.filter(h => h.level === 2).map((heading) => (
                    <a key={heading.id} href={`#${heading.id}`} className="block text-[12px] text-gray-400 hover:text-gray-900 transition-colors leading-relaxed py-0.5 border-l-2 border-transparent hover:border-gray-900 pl-3">{heading.text}</a>
                  ))}
                </nav>
              </aside>
            )}
              ...
              <div className="mt-12 pt-8 border-t border-gray-100">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {article.tags.map((tag) => (
                    <span key={tag} className="px-3 py-1 rounded-full bg-[#F7F7F5] text-[12px] text-gray-500">#{tag}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-10 grid sm:grid-cols-2 gap-4">
                {prevArticle ? (
                  <Link href={`/blog/${prevArticle.slug}`} className="group block p-5 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors">
                    <p className="text-[12px] text-gray-400 mb-1.5 flex items-center gap-1"><ArrowLeft size={12} /> Article précédent</p>
                    <p className="text-[14px] font-semibold text-gray-900 group-hover:text-gray-600 transition-colors leading-snug">{prevArticle.title}</p>
                  </Link>
                ) : <div />}
                {nextArticle && (
                  <Link href={`/blog/${nextArticle.slug}`} className="group block p-5 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors text-right">
                    <p className="text-[12px] text-gray-400 mb-1.5 flex items-center gap-1 justify-end">Article suivant <ArrowRight size={12} /></p>
                    <p className="text-[14px] font-semibold text-gray-900 group-hover:text-gray-600 transition-colors leading-snug">{nextArticle.title}</p>
                  </Link>
                )}
              </div>
            </div>
      ...
      <section className="py-16 bg-gray-900">
        <div className="container">
          <motion.div className="max-w-2xl mx-auto text-center" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp}>
            <h3 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight mb-4">Prêt à optimiser votre stratégie ?</h3>
            <p className="text-gray-400 mb-6 max-w-md mx-auto">Essayez Calenji gratuitement et planifiez vos publications sur 5 réseaux sociaux.</p>
        ...
        <div className="container">
          <div className="max-w-5xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              ...
              <Link href="/blog" className="text-[13px] text-gray-400 hover:text-gray-900 transition-colors flex items-center gap-1">Voir tous <ArrowRight size={14} /></Link>
            </div>
                ...
                <Link key={a.slug} href={`/blog/${a.slug}`} className="block group">
                  <div className="rounded-xl border border-gray-100 overflow-hidden hover:border-gray-200 transition-colors">
                    ...
                    <div className="p-4">
                      ...
                      <h3 className="text-[14px] font-semibold text-gray-900 mt-1 group-hover:text-gray-600 transition-colors leading-snug">{a.title}</h3>
